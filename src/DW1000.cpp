...
// SPI settings
	...
	SPISettings DW1000Class::_fastSPI = SPISettings(20000000L, MSBFIRST, SPI_MODE0);
#else
	SPISettings DW1000Class::_fastSPI = SPISettings(16000000L, MSBFIRST, SPI_MODE0);
#endif
...
void DW1000Class::readBytes(byte cmd, uint16_t offset, byte data[], uint16_t n) {
	...
	bus->beginTransaction(*_currentSPI);
	digitalWrite(_ss, LOW);
	for(i = 0; i < headerLen; i++) {
		bus->transfer(header[i]); // send header
	}
	for(i = 0; i < n; i++) {
		data[i] = bus->transfer(JUNK); // read values
	}
	delayMicroseconds(5);
	digitalWrite(_ss, HIGH);
	bus->endTransaction();
...
void DW1000Class::writeBytes(byte cmd, uint16_t offset, byte data[], uint16_t data_size) {
	...
	for(i = 0; i < data_size; i++) {
		bus->transfer(data[i]); // write values
	}
	delayMicroseconds(5);
	digitalWrite(_ss, HIGH);
	bus->endTransaction();
